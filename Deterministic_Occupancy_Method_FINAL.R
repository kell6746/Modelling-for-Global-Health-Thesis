...
Prob_ICU <- 0.08061                   #Probability that a patient will be admitted to ICU
Prob_H <- 1-0.08061                   #Probability that a patient will be admitted to the general ward

occupancy_day <- rep(0, length(data) + LoS_ICU_H + LoS_ICU)  #
occupancy_day

occupancy_day_H <- occupancy_day       #Hospitalisations without ICU admission 
occupancy_day_ICU <-occupancy_day      #ICU admissions
occupancy_day_H_ICU <-occupancy_day    #Additional time spent in general ward by ICU patients

for (dd in seq_along(data)){ #Loop for days
  
  print(paste0("#admissions: ", data[dd]))
    
    p_day_H <- data[dd] * Prob_H
    p_day_ICU <- data[dd] - p_day_H
    p_day_H_ICU <- p_day_ICU * 1        # 1 - Probability Stay in General Ward after ICU 

    occupancy_day_H[dd: (dd + LoS_H - 1)] <- occupancy_day_H[dd: (dd + LoS_H -1)] + p_day_H
    
    occupancy_day_ICU[dd: (dd + LoS_ICU - 1)] <- occupancy_day_ICU[dd: (dd + LoS_ICU -1)] + p_day_ICU
    occupancy_day_H_ICU[(dd + LoS_ICU):(dd + LoS_ICU + LoS_ICU_H - 1)] <-
      occupancy_day_H_ICU[(dd + LoS_ICU):(dd + LoS_ICU + LoS_ICU_H - 1)] + p_day_H_ICU
    
    occupancy_day_H[(dd + LoS_ICU):(dd + LoS_ICU + LoS_ICU_H - 1)] <-
      occupancy_day_H[(dd + LoS_ICU):(dd + LoS_ICU + LoS_ICU_H - 1)] + p_day_H_ICU
    
    occupancy_day[dd:(dd + LoS_H - 1)] <- occupancy_day[dd:(dd + LoS_H - 1)] + p_day_H
    occupancy_day[dd:(dd + LoS_ICU - 1)] <- occupancy_day[dd:(dd + LoS_ICU - 1)] + p_day_ICU
    occupancy_day[(dd + LoS_ICU):(dd + LoS_ICU + LoS_ICU_H - 1)] <-
      occupancy_day[(dd + LoS_ICU):(dd + LoS_ICU + LoS_ICU_H - 1)] + p_day_H_ICU
...
occupancy_day_ICU
occupancy_day_H_ICU
occupancy_day_H_V <- occupancy_day_H * 0.1032  #Probability that a general ward patient will be ventilated

plot (occupancy_day)
plot (occupancy_day_H)
plot (occupancy_day_ICU)
plot (occupancy_day_H_V)

df_total <- data.frame(occupancy_day)
df_H <- data.frame(occupancy_day_H)
df_ICU <- data.frame(occupancy_day_ICU)
df_H_V <- data.frame(occupancy_day_H_V)
days<-(1:389)

df_all <- cbind(df_total, df_H, df_ICU, df_H_V, days)
  ...
  geom_line(data = df_total, aes(x = days, y = occupancy_day_H_V, colour = "yellow")) +
  labs(title = "Mexico COVID-19 Estimated Hospital Occupants",
       subtitle = "March 2020 to March 2021",
       x = "Day",
       y = "Beds Occupied") +
  geom_density()+
  scale_color_manual(
    labels=c("Total Beds", "General Ward Beds Occupied", "ICU Beds Occupied", "General Ward Admissions with Ventilation"),
...
#Count hospital admissions per day
...
#Plotting Admission in Mexico City per day
ggplot(ADMITTED_PATIENTS_CDMX, aes(x = date, y = Admitted)) +
  geom_line(color = "red", 
            size=1 ) +
  scale_x_date(date_breaks = '1 month', 
               labels = date_format("%b-%y")) +
  labs(title = "Reported Mexico City COVID-19 Hospital Admissions",
...
Prob_H <- 1-0.08061                   #Probability that in patient will be Admitted to General Ward
...
ggplot() + 
  ...
  geom_line(data = df_H, aes(x = days, y = occupancy_day_H, colour = "orange")) +
  geom_line(data = df_ICU, aes(x = days, y = occupancy_day_ICU, colour = "red")) +
  labs(title = "Mexico City COVID-19 Estimated Hospital Occupants",
       subtitle = "March 2020 to March 2021",
       x = "Day",
       y = "Beds Occupied",
       colour = "Colour") +
  theme_minimal()+
  scale_color_manual(
    labels=c("Total Beds", "General Ward Beds Occupied", "ICU Beds Occupied"),
